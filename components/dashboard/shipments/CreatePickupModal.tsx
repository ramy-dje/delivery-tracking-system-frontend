...
import { Phone, MapPin, DollarSign, FileText, Weight, X } from "lucide-react";
import InputField from "@/components/commons/InputField";
import EntityPicker from "@/components/commons/EntityPicker";
import { ICreatePickupShipment } from "@/types/shipment";
...
interface Props {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: ICreatePickupShipment) => Promise<void>;
    loading?: boolean;
    merchantId: string;
}
...
export default function CreatePickupModal({ isOpen, onClose, onSubmit, loading, merchantId }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const form = { customerName, phoneNumber, communeId, destinationHubId, codAmount, deliveryFee, description, weightKg };
        const v = validate(form);
        if (Object.keys(v).length) { setErrors(v); return; }
        try {
            const payload: ICreatePickupShipment = {
                merchantId,
            ...
            showToast.success("Pickup shipment created");
            handleClose();
        } catch (err: any) {
            const e = parseApiError(err);
            showToast.error(e.message || "Failed to create pickup shipment");
        }
    };

    const handleClose = () => {
        setCustomerName(""); setPhoneNumber(""); setCommuneId(null); setDestinationHubId(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-2xl mx-4 rounded-2xl border border-white/10 bg-linear-to-br from-slate-900/95 to-slate-950/95 shadow-2xl">
                <div className="px-6 py-4 border-b border-white/5 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-slate-100">Create Pickup Shipment</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
                    <div className="space-y-3">
                        ...
                        <InputField label="Full Name" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Customer full name" icon={Phone} error={errors.customerName} />
                        <InputField label="Phone Number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="+213..." icon={Phone} error={errors.phoneNumber} />
                        <div>
                    ...
                    <div className="space-y-3">
                        ...
                        <InputField label="Description (Optional)" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Package contents, special instructions..." icon={FileText} />
                    ...
                    <div className="flex gap-3 pt-4 border-t border-white/5">
                        ...
                        <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-medium transition-colors">{loading ? "Creating..." : "Create Pickup"}</button>
                    </div>
                </form>
            </div>
        </div>
