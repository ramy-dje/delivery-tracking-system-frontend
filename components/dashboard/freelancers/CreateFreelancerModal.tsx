...
import { Mail, Phone, User, Lock, UserCheck } from "lucide-react";
import InputField from "@/components/commons/InputField";
import { ICreateFreelancer } from "@/types/freelancer";
...
interface CreateFreelancerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: ICreateFreelancer) => Promise<void>;
  loading?: boolean;
}

export default function CreateFreelancerModal({ isOpen, onClose, onSubmit, loading }: CreateFreelancerModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    await onSubmit({ firstName: firstName.trim(), lastName: lastName.trim(), email: email.trim(), password, phone: phone.trim() });
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[14px] font-semibold text-white">Register Freelancer</div>
              <div className="text-[11px] text-slate-600">Add a new freelance delivery agent to this branch</div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 px-6 py-4 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
          ...
          <button type="button" onClick={handleSubmit} disabled={loading}
            ...
            {loading
              ...
              : <><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" /></svg>Register Freelancer</>
            }
          </button>
        </div>
      </div>
